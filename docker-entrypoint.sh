...
function validate_envvars() {
    required_vars=("SMTP_HOST" "SMTP_USER" "SMTP_PASSWORD" "ROOT_FORWARD" "CONTAINER_HOSTNAME")
    required_vars_values=("$SMTP_HOST" "$SMTP_USER" "$SMTP_PASSWORD" "$ROOT_FORWARD" "$CONTAINER_HOSTNAME")

	total=${#required_vars[*]}
	
	for (( i=0; i<=$(( total -1 )); i++ ))
	do 
        if [ -z "${required_vars_values[$i]}" ]; then
            echo  "environment variable ${required_vars[$i]} is empty"
            exit 1
        fi 
	done

	if [ "$(hostname -f)" != "$CONTAINER_HOSTNAME" ]; then 
		echo "hostname (set with docker run -h) and CONTAINER_HOSTNAME must be equal"
		exit 1
	fi 
}
